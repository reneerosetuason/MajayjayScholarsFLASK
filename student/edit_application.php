...
$stmt = $db->prepare('SELECT * FROM application WHERE application_id = ? AND user_id = ? LIMIT 1');
...
if (strtolower($appData['status']) !== 'pending') {
    flash('Only pending applications can be edited.', 'error');
    redirect('my_applications.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateColumns = 'student_id=?, contact_number=?, address=?, municipality=?, baranggay=?, school_name=?, course=?, year_level=?, gwa=?, year_applied=?, reason=?';
    $params = [$student_id, $contact_number, $address, $municipality, $barangay, $school_name, $course, $year_level, $gwa, $year_applied, $reason];
    ...
    $stmt = $db->prepare("UPDATE application SET {$updateColumns} WHERE application_id = ? AND user_id = ?");
    ...
    $stmt->close();
    flash('❌ Unable to update your application. Please try again.', 'error');
}
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Edit Application | Scholar App</title>
...
<body>
    ...
    <div class="navbar">
        <h1><button class="toggle-btn" onclick="toggleSidebar()">☰</button>✏️ Edit Scholarship Application</h1>
    </div>
    <div class="page">
        <div class="header-card">
            <h2>Edit Application</h2>
            <p>Update your pending application details and upload any revised documents.</p>
        </div>
        ...
        <div class="form-card">
            <div class="info-box">
                Only pending applications can be edited. Uploaded files are optional unless you want to replace them.
            </div>
            <form action="edit_application.php?id=<?= h($appId) ?>" method="POST" enctype="multipart/form-data">
                <div class="section-title">👤 Personal Details</div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="student_id">Student ID</label>
                        <input id="student_id" name="student_id" type="text" value="<?= h($appData['student_id']) ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="contact_number">Contact Number</label>
                        <input id="contact_number" name="contact_number" type="tel" value="<?= h($appData['contact_number']) ?>" required>
                    </div>
                </div>
                <div class="form-group">
                    <label for="address">Address</label>
                    <input id="address" name="address" type="text" value="<?= h($appData['address']) ?>" required>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="municipality">Municipality</label>
                        <input id="municipality" name="municipality" type="text" value="<?= h($appData['municipality']) ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="barangay">Barangay</label>
                        <input id="barangay" name="barangay" type="text" value="<?= h($appData['baranggay']) ?>" required>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="school_name">School Name</label>
                        <input id="school_name" name="school_name" type="text" value="<?= h($appData['school_name']) ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="course">Course</label>
                        <input id="course" name="course" type="text" value="<?= h($appData['course']) ?>" required>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="year_level">Year Level</label>
                        <input id="year_level" name="year_level" type="text" value="<?= h($appData['year_level']) ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="gwa">GWA</label>
                        <input id="gwa" name="gwa" type="text" value="<?= h($appData['gwa']) ?>" required>
                    </div>
                </div>
                <div class="form-group">
                    <label for="year_applied">Year Applied</label>
                    <input id="year_applied" name="year_applied" type="number" value="<?= h($appData['year_applied']) ?>" required>
                </div>
                <div class="form-group">
                    <label for="reason">Reason</label>
                    <textarea id="reason" name="reason" required><?= h($appData['reason']) ?></textarea>
                </div>
